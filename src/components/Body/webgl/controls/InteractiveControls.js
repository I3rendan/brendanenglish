...
export default class InteractiveControls extends EventEmitter {
	...
	constructor(camera, el) {
		...
		// setTimeout(() => this.resize(0, 0, window.innerWidth, window.innerHeight));
	}
	...
	onMove(e) {
		...
		this.mouse.x = ((touch.x + (this.rect.x - this.rect.left)) / this.rect.width) * 2 - 1;
		this.mouse.y = -((touch.y + (this.rect.y - this.rect.top)) / this.rect.height) * 2 + 1;
